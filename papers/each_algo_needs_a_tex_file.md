...
- and do not link to "examples" (example --> concrete applications to real data, tutorials --> make some important point to learn, 1:1; this primer_example is different, its a quick check on a simulated case to show flawless recovery / acpplication. the primer_exmaple is chosen based on the paper itself. we need to at least replication one small thing from the paper that proves a) the algo works and b) teh algo solves the main problem it is designed to solve -- just 1 primer_exmaple will do, but it has to be the right one, and we have to go all the way in running it (inclding its key competitior and showing exactly the conditions whern this estimator wins). remeber dont show the case where the estimator loses (standard bus engine or soem random mdp or gridworld), show na exampl where it wins and improves on its clearest benchmark. remember WINS by a mile and its not even funny, not draw or lose. WINS --> where its advantages are showcased in the way the authors intended. a steelman. if the estimator can do continous control or high dim reward then we need to tailor the exasmpel to that. remmeber its a simulated exmpale, so we control the setting compeltely. 
...
- Comparisons with other estimators need to breif, and no drawing up tables. Compare with classic, structural and IRL estimators. 
...
- after you are done please update the read the docs (the estimator page is a "just what you need to know to run it + code snippet with output; adn with the github pdf link to the primer right on top; and the main estimator tab needs the github link too; organize into tables)
- all above things HAVE to be included, please run a checklist. 
- at the end do read the paper again and run a final check against the algo pseudo code to see if we got everything right. 
