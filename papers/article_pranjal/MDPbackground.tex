...
This is a class of stochastic iterative algorithms for approximating the value function $V$ and the optimal
policy $\pi$ inspired by models of trial and error learning by animals and humans. One of the first RL methods was 
{\it temporal difference learning\/} (TD) \citet{sutton:1988} which can be regarded as a type of 
{\it stochastic approximation\/} \citep{robbinsmunro:1951} for solving the linear system (\ref{eq:policy_valuation}) for
$V_\delta$ in the policy valuation step of PI. TD uses simulated or realized outcomes from $p(s'|s,\delta(s))$
in an iterative updating process for $V_\delta$ rather than numerical
integration to compute the expectation operator $E_\delta$ in (\ref{eq:policy_valuation})
and finding $V_\delta$ in a single step by solving a linear system with $|S|$ equations and unknowns.
Instead TD requires many light iterations $t$ that result in a stochastic sequence $\{\tilde V^t_\delta\}$ 
which is updated only at the realized state $s_t$ using the random successor state $\tilde s_{t+1} \sim p(s'|s_t,\delta(s_t))$ via the
recursion
\begin{equation}
              \tilde V^{t+1}_\delta(s_t)= \tilde V^t_\delta(s_t)+ \alpha_t\left[r(s_t,\delta(s_t))+\beta \tilde V^t_\delta(\tilde s_{t+1})-
\tilde V^t_\delta(s_t)\right],
\label{eq:td}
\end{equation}
where $\alpha_t \ge 0$ is a step-size or learning rate sequence that must tend to zero, though not too fast. Under certain conditions
on $\{\alpha_t\}$ the sequence of values $\{\tilde V^t_\delta\}$ from (\ref{eq:td})
 converges with probability 1 to the vector $V_\delta$ that solves the system $V_\delta=r_\delta+
\beta E_\delta V_\delta$.

Another example is {\it Q-learning\/} \citep{Watkins:1989}, originally formulated as ``a general
formal model of an animal's behavioural choices in its environment.'' 
The $Q$ function is the {\it choice-specific value function\/} implied by the  Bellman equation
\begin{eqnarray}
      Q(s,a) &=& r(s,a) + \beta \sum_{s'} V(s')p(s'|s,a), \label{eq:q1}   \\
      V(s) &=& \max_{a \in A(s)} Q(s,a).
\label{eq:q2}
\end{eqnarray}
Watkins proposed a stochastic iterative algorithm for updating $Q$ given by
\begin{equation}
...
$p(s'|s_t,a_t)$. Q-learning is also a form of stochastic approximation and \citet{Tsitsiklis:1994}
