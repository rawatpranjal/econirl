...
\citet{ziebart2010} introduced Maximum Causal Entropy IRL (MCE-IRL), which conditions each action only on information available at decision time by replacing Shannon entropy $H(A^T)$ with causal entropy $H(A_t | S_{1:t}, A_{1:t-1})$. This avoids non-causal visitation calculations that can propagate future information backward. The resulting softmax Bellman equation is simultaneously the MCE optimal policy (Theorem 1 of \citealt{ziebart2010}) and the structural logit choice model of \citet{rust1987}, making MCE-IRL the bridge between econometrics and IRL.
...
The validation uses the shared known-truth DGP rather than a real-data
example. The DGP supplies the true reward, transition law, value function,
policy, occupancy measures, and Type A/B/C counterfactual oracles. The success
claim is therefore structural recovery under known transitions and known
action-dependent reward features, not a high likelihood value by itself.

Reward, value, and \(Q\) recovery are reported after the standard IRL
location-and-scale normalization. The policy and counterfactual gates are not
normalized: they test whether the recovered reward actually induces the right
behavior in the baseline and intervention environments. Raw parameter cosine is
not used as a gate for MCE-IRL.

\paragraph{Code.}
The companion script \texttt{mce\_irl\_run.py} in this folder generates the
JSON and \LaTeX{} artifacts. The MCE-IRL estimator is implemented in
\texttt{src/econirl/estimation/mce\_irl.py}. To regenerate:
\begin{verbatim}
