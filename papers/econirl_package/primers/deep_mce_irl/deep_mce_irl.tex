...
\max_\pi \; \E_\pi\!\left[\sum_{t=0}^{\infty} \beta^t \bigl(R_\psi(s_t, a_t) + \sigma \calH(\pi(\cdot | s_t))\bigr)\right]
\end{equation}
...
The Objectworld has only 64 states, so the neural network has far more parameters than needed, and the occupancy mismatch provides only a 320-dimensional gradient signal per iteration. The neural network overfits to noise in the occupancy rather than learning the true reward structure. The projection $R^2$ of \deepDeepProjectionR\ confirms this. Linear MCE-IRL outperforms the deep variant here because 2 linear parameters cannot overfit, while the deep model has thousands of parameters and insufficient training signal. Deep MCE-IRL is expected to outperform linear MCE-IRL on larger state spaces (32x32 or larger) where nonlinear reward structure matters more.
