...
class TestNeuralCostFunction:
    ...
    def test_neural_cost_forward_single(self):
        """Test forward pass with single state-action pair."""
        ...
        assert cost.dim() == 0, "Single input should return scalar"
        assert torch.isfinite(cost), "Cost should be finite"
    ...
    def test_neural_cost_forward_batch(self):
        ...
        assert torch.all(torch.isfinite(costs)), "All costs should be finite"
    ...
    def test_neural_cost_compute_shape(self):
        ...
        assert torch.all(torch.isfinite(cost_matrix)), "All costs should be finite"
    ...
    def test_neural_cost_compute_consistency(self):
        ...
        for s in range(n_states):
            for a in range(n_actions):
                expected = cost_fn(torch.tensor(s), torch.tensor(a))
                actual = cost_matrix[s, a]
                assert torch.allclose(expected, actual, atol=1e-6), f"Mismatch at ({s}, {a})"
    ...
    def test_neural_cost_reward_matrix(self):
        ...
        assert torch.allclose(reward_matrix, -cost_matrix), "Reward should be negative cost"

    def test_neural_cost_gradient_enabled(self):
        """Test that cost function supports gradient computation."""
        cost_fn = NeuralCostFunction(n_states=10, n_actions=2, embed_dim=8, hidden_dims=[16])
        ...
        for param in cost_fn.parameters():
    ...
    def test_neural_cost_activations(self):
        ...
        for activation in ["relu", "tanh", "leaky_relu"]:
            ...
            assert torch.all(torch.isfinite(cost_matrix))
...
class TestGCLEstimatorHelpers:
    ...
    def simple_setup(self):
        ...
        for s in range(n_states):
            if s < n_states - 1:
                ...
                transitions[0, s, s] = 1.0  # stay at terminal
            transitions[1, s, 0] = 1.0  # replace -> go to 0
    ...
    def test_trajectory_cost_computation(self, simple_setup):
        ...
        expected_cost = sum(
            cost_fn(torch.tensor(s), torch.tensor(a))
            for s, a in zip(traj.states.tolist(), traj.actions.tolist())
        )

        assert torch.allclose(total_cost, expected_cost, atol=1e-6), "Trajectory cost should sum individual costs"
    ...
    def test_trajectory_log_prob(self, simple_setup):
        ...
        assert torch.isclose(log_prob, torch.tensor(expected, dtype=torch.float32), atol=1e-6), "Log prob should be sum of log action probs"
    ...
    def test_importance_weights_normalized(self, simple_setup):
        ...
        trajectories = [
            ...
            Trajectory(
                ...
                next_states=torch.tensor([0, 2]),
            ),
            Trajectory(
                states=torch.tensor([2, 3, 4]),
                actions=torch.tensor([0, 0, 1]),
                next_states=torch.tensor([3, 4, 0]),
            ),
        ]
        ...
        assert torch.isclose(weights.sum(), torch.tensor(1.0), atol=1e-6), "Weights should sum to 1"
    ...
    def test_importance_weights_clipping(self, simple_setup):
        ...
        assert torch.all(weights <= max_weight + 1e-6), f"Weights should be clipped to {max_weight}"
    ...
    def test_trajectory_sampling(self, simple_setup):
        ...
        policy = torch.ones((n_states, n_actions)) / n_actions
        initial_dist = torch.ones(n_states) / n_states
        ...
        for traj in trajectories:
            ...
            assert torch.all(traj.states >= 0) and torch.all(traj.states < n_states), "States should be valid"
            assert torch.all(traj.actions >= 0) and torch.all(traj.actions < n_actions), "Actions should be valid"
    ...
    def test_trajectory_sampling_respects_transitions(self, simple_setup):
        ...
        for traj in trajectories:
            ...
            assert traj.states[0].item() == 0, "Should start at state 0"
            # Check transition consistency
            for t in range(len(traj) - 1):
                s = traj.states[t].item()
                a = traj.actions[t].item()
                s_next = traj.next_states[t].item()
                # Check that this transition has non-zero probability
                assert transitions[a, s, s_next] > 0, f"Invalid transition from {s} with action {a} to {s_next}"
    ...
    def test_initial_distribution_from_panel(self, simple_setup):
        ...
        # Should sum to 1
        assert torch.isclose(initial_dist.sum(), torch.tensor(1.0)), "Initial dist should sum to 1"

        # State 0 should have all the mass (both trajectories start there)
        assert initial_dist[0] == 1.0, "All trajectories start at state 0"
...
class TestGCLEstimatorOptimization:
    ...
    def test_gcl_policy_is_valid_distribution(self):
        ...
        assert torch.allclose(policy_sums, torch.ones(n_states), atol=1e-6), "Policy should be valid distribution"

        # Policy should be non-negative
        assert torch.all(result.policy >= 0), "Policy probabilities should be non-negative"
